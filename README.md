...
```shell
npm install https://github.com/semanticsAnonymous/rmlmapper-js
```

```shell
yarn add https://github.com/semanticsAnonymous/rmlmapper-js
```
...
```typescript
import * as RmlMapper from "rmlmapper-js";

const inputFiles = {
  "input.json": '{ "name": "semanticsAnonymous" }',
};
...
const turtleMappingResult = await RmlMapper.parseTurtle(
  turtleMapping,
  inputFiles,
  options
);
...
const jsonLdMappingResult = await RmlMapper.parseJsonLd(
  jsonLdMapping,
  inputFiles,
  options
);
...
const options = {
  functions: {
    "http://users.ugent.be/~bjdmeest/function/grel.ttl#createDescription": (
      data
    ) => {
